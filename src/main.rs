...
impl Table {
    ...
    pub fn push(&mut self, filename: &str, cnt: u8) {
        ...
        // debug info
        println!("push");
        println!("input: {:?}", (Reverse(cnt), filename.to_string()));
        println!("{:?}", self.hashtable);
        println!("{:?}", self.pq);
    }
    ...
    pub fn is_empty(&self) -> bool {
        println!("is_empty() : {:?}", self.pq.is_empty());
        self.pq.is_empty()
    }

    pub fn len(&self) -> usize {
        println!("len() : {:?}", self.pq.len());
        self.pq.len()
    }

    pub fn contains(&self, filename: &str) -> bool {
        println!("contains({:?}) : {:?}", filename, self.hashtable.contains_key(filename));
